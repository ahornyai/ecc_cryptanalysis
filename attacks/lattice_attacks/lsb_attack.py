...
class LSBAttack(ECDSAttack):
    ...
    def attack(self, signatures, pubkey, curve, generator) -> int | None:
        known_part_length = self.find_known_part_length(signatures)
        order = curve.order()
        sample_size = len(signatures["r"])
        s_inverse = [pow(s, -1, order) for s in signatures["s"]]
        unknown_nonce_bits = curve.order().nbits() - known_part_length

        offset = 2**(unknown_nonce_bits-1)
        offset_vec = vector([offset] * sample_size)

        A = vector([((pow(2**known_part_length, -1, order)) * (s_inv * r)) % order for (s_inv, r) in zip(s_inverse, signatures["r"])])
        B = vector([((pow(2**known_part_length, -1, order)) * (kp - s_inv * h)) % order for (s_inv, h, kp) in zip(s_inverse, signatures["h"], signatures["kp"])])
        ...
        sig_k = [(kp - k * 2**known_part_length) % order for (k, kp) in zip(shortest, signatures["kp"])]
